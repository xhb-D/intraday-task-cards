...
import { LEGACY_INTRADAY_KEY, LEGACY_RISK_KEY, PRE_IMPORT_KEY, UNIFIED_KEY, classifyBackup, commitUnified, loadUnified, makeUnified, normalizeImport, validateRisk } from '../src/unified-persistence.js';
...
test('C01/C02/C03/C04: legacy keys migrate without mutation and canonical wins', () => {
  const intraday = JSON.stringify(makeEnvelope(createWorkspace(1), 1)); const s = store({ [LEGACY_INTRADAY_KEY]: intraday });
  const loaded = loadUnified(s); assert.equal(loaded.source, 'legacy'); assert.equal(s.getItem(LEGACY_INTRADAY_KEY), intraday); assert.deepEqual(loaded.state.sections.riskManager, risk());
...
test('C12/C13: partial import leaves the other section and preferences untouched', () => {
  const current = makeUnified(makeEnvelope(createWorkspace(1), 1), risk(), { appearance: 'dark' });
  const intraday = makeEnvelope(createWorkspace(2), 2); const imported = normalizeImport(intraday, current).state;
  assert.deepEqual(imported.sections.riskManager, current.sections.riskManager); assert.deepEqual(imported.preferences, current.preferences);
...
test('C19/C20/C21/C22/C23/C24: classifier and strict risk validation fail closed', () => {
  assert.throws(() => classifyBackup({}), /无法识别/); assert.throws(() => classifyBackup({ app: 'intraday-task-cards', state: {}, schemaVersion: 2, accounts: [], selectedAccountId: null }), /歧义/);
