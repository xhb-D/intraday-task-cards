...
import { chooseSetup, changeDirection } from '../src/model.js';
...
test('startup: getItem 抛异常时仍返回可操作的 GC/CL/ES 内存工作区', () => {
  ...
  assert.equal(changeDirection(result.state, 'GC', 'long', time + 1).changed, true);
  assert.equal(chooseSetup(result.state, 'GC', 'pullback', time + 2).changed, true);
});
...
test('startup: setItem 抛异常不丢失当前内存状态，保存结果明确失败且后续状态机仍可用', () => {
  ...
  assert.equal(changeDirection(initial.state, 'CL', 'short', time + 2).changed, true);
});
