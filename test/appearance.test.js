...
test('appearance: head 在 CSS 前同步应用偏好，控件不进入交易备份', () => {
  ...
  const bootstrap = readFileSync(new URL('../appearance-bootstrap.js', import.meta.url), 'utf8');
  assert.ok(html.indexOf('appearance-bootstrap.js') < html.indexOf('styles.css'));
  assert.match(html, /id="appearance-select"/);
  assert.match(bootstrap, /trading-risk-manager:appearance/);
  assert.doesNotMatch(bootstrap, /intraday-task-cards:v3/);
});
