...
test('production bundle: index 使用 classic bundle，包含统一持久化与双路由且不依赖网络', () => {
  ...
  assert.match(html, /<script src="dist\/app\.bundle\.js\?v=unified-20260910-2"><\/script>/);
  assert.match(html, /<link rel="stylesheet" href="banner\.css\?v=unified-20260910-2">/);
  assert.equal((html.match(/\?v=unified-20260910-2/g) || []).length, 7, '全部脚本和样式使用同一发布版本，避免新旧资源混载');
